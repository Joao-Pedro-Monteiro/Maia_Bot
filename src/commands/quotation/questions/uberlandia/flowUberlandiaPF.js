...
class FlowUberlandiaPF {
    static async iniciar(state) {
        console.log("Cotação Uberlândia PF, iniciada ......\n....\n..");
    ...
    static processarPlano(userInput, state) {
        ...
        if (userInput === '1') {
            ...
            const valorPlano = tabelaHappyVidaPF.Uberlandia[cliente.cobertura]['Planos_Odontológicos'];
    ...
    static processarIdades(userInput, state) {
        ...
        try {
            ...
            Scout.recordFailure('flow_error_Uberlandia_pf');
    ...
    static processarConfirmacao(userInput, state) {
        ...
        if (userInput.toLowerCase() === 's') {
            console.log("Cotação concluída com sucesso!\nEncerrando cálculo de cotação para PF em Uberlândia... \n\n");
    ...
    static obterTabelaPrecos(cliente) {
        try {
            ...
            if (cliente.plano === 'Plano Odontológico') {
                return { valor: tabelaHappyVidaPF.Uberlandia[cliente.cobertura]['Planos_Odontológicos'] };
            }
            
            // Para outros planos
            try{
                basePath = tabelaHappyVidaPF.Uberlandia[cliente.cobertura];
            }catch(error){
            ...
            if (cliente.cobertura === 'Ambulatorial') {
                tabela = basePath[cliente.plano][cliente.coparticipacao].SACOM[cliente.assistenciaTabela];
            } else {
...
module.exports = FlowUberlandiaPF;